 ...
 * 구현한 "그림자 파일"들이 있다. 원작(src/main) 쪽이 바뀌면 웹 포트도 검토가 필요하지만,
 * 그 어긋남은 타입체크·빌드로는 잡히지 않는다 (동작 변경이므로). 이 스크립트는 감시 대상
 * 원작 파일의 해시를 drift-sentinel.json과 대조해, 변경이 있으면 어떤 웹 파일을
 * 검토해야 하는지 알려주고 실패(exit 1)한다. CI(web-check.yml)에서 실행된다.
 *
...
console.error(
  '[web-drift] 원작 파일이 변경되었습니다. 웹 포트(src/web)의 대응 구현 검토가 필요합니다.\n'
)
